...
test('submitRecipe() on a second recipe adds to the recipes value in state', () => {
  const wrapper = shallow(<App />)
  const recipeName = "Hot Pockets"
  const recipeInstructions = "microwave for 60 seconds"
  wrapper.setState({
  ...
  const recipeName2 = "More food"
  const recipeInstructions2 = "More instructions"
  wrapper.setState({
    isAddRecipeFormDisplayed: true,
    newRecipeName: recipeName2,
    newRecipeInstructions: recipeInstructions2
  })
  ...
  expect(mockPreventDefault).toHaveBeenCalled()
  expect(wrapper.state().recipes[0]).toEqual(submittedRecipe)
  expect(wrapper.state().recipes[1]).toEqual(submittedRecipe2)
})